...
//    tf_prefix_ = kinova_robotType_ + "_" + boost::lexical_cast<string>(same_type_index); // in case of multiple same_type robots
    ...
    sensor_info_publisher_ = node_handle_.advertise<kinova_msgs::SensorInfo>
            ("out/sensors_info", 2);
...
/*!
 * \brief Publishes the sensors informations such as current, temperature and so on.
 */
void KinovaArm::publishSensorsInfo(void)
{
    KinovaSensors kinova_sensors;
    SensorsInfo sensors;

    kinova_comm_.getSensorsInfo(sensors);
    kinova_sensors = KinovaSensors(sensors);
    sensor_info_publisher_.publish(kinova_sensors.constructSensorsMsg());
}
