...
class PickPlaceServer(object):
    ...
    def __init__(self):
        ...
        rospy.init_node('Pickplace_server_node')
        ...
        self.stop = rospy.Subscriber('stop_move', std_msgs.msg.String, callback=self.stop_cb)
        ...
        self.arm.remember_joint_values('initial_pose')
    ...
    def pick_and_place(self, goal):
        ...
        rospy.logdebug("grasp_pose:\n%s", grasp_pose)
        rospy.logdebug("object_size:\n%s", target_size)
    ...
    def back_to_init_pose(self, state=0):
        ...
        if state:
            self.gripper.set_named_target("Open")
            self.gripper.go()
        self.arm.set_named_target('initial_pose')
    ...
    def stop_cb(feedback):
        rospy.logdebug(feedback.data)
        self.arm.stop()
        self.gripper.stop()
        rospy.sleep(0.1)
